...
PART 4: FLOW DIAGRAM (Request → Response)
text
CLIENT
  │
  ▼
API GATEWAY  (routes /restaurant/** to restaurant-service:9091)
  │
  ▼
CorrelationIdFilter  (@Order(1))
  │  - read/generate X-Correlation-Id
  │  - MDC.put("correlationId", id)
  ▼
RestaurantController
  │  - parse request (path/query/body/multipart)
  │  - call service
  ▼
RestaurantServiceImpl
  │
  ├── @Cacheable  ────► Redis  (get)
  ├── @CacheEvict ────► Redis  (wipe)
  ├── RestaurantRepository ──► MySQL (restaurantdb)
  └── FileStorageService
         │
         ├── init() @PostConstruct (startup)
         ├── storeFile()  ──► Disk  uploads/restaurants/{id}.{ext}
         ├── readFile()   ──► Disk
         └── deleteFile() ──► Disk
  │
  ▼
Response travels back up the chain
  │
  ▼
CorrelationIdFilter  finally { MDC.remove("correlationId") }
  │
  ▼
CLIENT
...
1. GET /fetchById/{id} — cache miss then hit
text
Client  Controller  Service  Redis  MySQL
  │        │          │       │      │
  │ GET ──►│          │       │      │
  │        │ fetch ──►│       │      │
  │        │          │ get ─►│      │
...
Next identical request:
  ...
  │        │◄── DTO ──│       │      │
  │◄─ 200 ─│          │       │      │
2. POST /addRestaurant
text
Client  Controller  Service  MySQL  Redis
  │        │          │       │      │
  │ POST ─►│          │       │      │
  │        │ add ────►│       │      │
  │        │          │ save ►│      │
  │        │          │◄ entity      │
  │        │          │ evict restaurantPage ►│
  │        │◄── DTO ──│       │      │
  │◄─ 201 ─│          │       │      │
3. POST /uploadImage/{id}
text
Client  Controller  Service  FileStorage  Disk  MySQL  Redis
  │        │          │          │          │      │      │
  │ file ─►│          │          │          │      │      │
  │        │ upload ─►│          │          │      │      │
  │        │          │ findById ───────────┼─────►│      │
  ...
  │        │          │◄── filename ────────│      │      │
  │        │          │ save ───────────────┼─────►│      │
  │        │          │ evict restaurant + restaurantPage ►│
  │        │◄── URL ──│          │          │      │      │
  │◄─ 200 ─│          │          │          │      │      │
4. DELETE /deleteRestaurant/{id}
text
Client  Controller  Service  FileStorage  Disk  MySQL  Redis
  │        │          │          │          │      │      │
  │ DEL ──►│          │          │          │      │      │
...
┌─────────────────────────────┐        ┌──────────────────────┐
│  <<interface>>              │        │  <<interface>>       │
│  RestaurantService          │        │  FileStorageService  │
│  + featchAllRestaurant()    │        │  + storeFile()       │
│  + addRestaurant()          │        │  + readFile()        │
│  + fetchRestaurantById()    │        │  + deleteFile()      │
│  + uploadRestaurantImage()  │        └──────────┬───────────┘
│  + getRestaurantImage()     │                   │ implemented by
│  + getDefaultImage()        │                   ▼
│  + updateRestaurant()       │      ┌──────────────────────────┐
│  + deleteRestaurant()       │      │ FileStorageServiceImpl   │
└──────────────┬──────────────┘      │ (@Service)               │
               │ implemented by      │ - uploadDir (@Value)     │
               ▼                     │ + init() @PostConstruct  │
┌─────────────────────────────┐      │ + storeFile()            │
│  RestaurantServiceImpl      │      │ + readFile()             │
│  (@Service)                 │      │ + deleteFile()           │
│  - RestaurantRepository     │      └────────────┬─────────────┘
│  - FileStorageService       │                   │
│  - baseImageUrl (@Value)    │                   ▼
│  + all methods above        │             Disk (uploads/)
└──────┬──────────────────────┘
       │ uses
       ▼
┌─────────────────────────────┐        ┌──────────────────────┐
│  RestaurantRepository       │        │  RestaurantMapper    │
│  extends JpaRepository      │        │  (MapStruct)         │
│  <Restaurant, Integer>      │        │  Entity ↔ DTO        │
└──────────────┬──────────────┘        └──────────────────────┘
               │
               ▼
┌─────────────────────────────┐
...
DTOs:
┌──────────────────────┐   ┌────────────────────────┐
│   RestaurantDTO      │   │  RestaurantPageDto     │
│   id, name, address, │   │  restaurantList,       │
│   city, description, │   │  pageNo, pageSize,     │
│   imageUrl, rating,  │   │  totalElement,         │
│   reviewCount        │   │  totalPage, isLast     │
└──────────────────────┘   └────────────────────────┘

Cross-cutting:
┌──────────────────────┐   ┌──────────────────────┐
│ CorrelationIdFilter  │   │   CacheConfig        │
│ (@Component, @Order) │   │ (@Configuration,     │
│ - MDC correlationId  │   │  @EnableCaching)     │
└──────────────────────┘   │  + cacheManager()    │
                           └──────────────────────┘

Exceptions:
┌──────────────────────────────┐   ┌──────────────────────────────┐
│ RestaurantNotFoundException  │   │ RestaurantServiceException   │
└──────────────────────────────┘   └──────────────────────────────┘
